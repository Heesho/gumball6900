...
> ADRs 0031, 0034, and 0036 define the development authority model below. Governance execution remains an unselected
...
2,000 basis points. Fund receives the complementary classification. A change affects only payments classified after
the setter transaction; existing liabilities, reward schedules, claims, and weighted split carry remain unchanged.
There is no per-Strategy rate or second Fund-share setter. At 0%, signal entry, movement, withdrawal, killed-Strategy
exit, existing reward settlement, and independent Bribe funding remain permissionless and live.
