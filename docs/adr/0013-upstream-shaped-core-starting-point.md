...
The first replacement draft overcorrected to six contracts. It omitted revenue and bribe routers, both factories, voter
...
`GBX`, `Fundraiser`, `LiquidityPosition`, `SignalGBX`, `VoterRouter`, `Voter`, `StrategyFactory`, `Strategy`,
...
- every Fundraiser contribution routes through VoterRouter into Voter;
- SignalGBX voting and withdrawal have no time-based restriction;
- Voter alone uses the Strategy and Bribe factories;
- acquisitions begin with a 90% Fund / 10% voter-reward split, adjustable up to 50% through timelocked governance;
- buybacks burn their entire GBX payment and do not pay voter rewards;
...
- Voter and Fund administration use OpenZeppelin `TimelockController`, with the project multisig as proposer and
...
- Unrestricted voting favors simplicity and liquidity over epoch-level allocation stability.
