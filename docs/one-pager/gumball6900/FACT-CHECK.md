...
| G-03 | "Every holder's sGBX is pooled, so the next dollar released splits the same way. Move yours any time." | `Resonance._checkpointRevenue` allocates elapsed USDG by each Strategy's share of `totalSignalWeight`; no cooldown or epoch exists | `Resonance.t.sol`; `Invariants.t.sol` accounting identity; `invariant_EveryActorCanUnstakeItsUnallocatedBalance` | Yes      | Unindexable old-weight carry goes to Fund before the denominator changes. Idle sGBX directs nothing and does not dilute active signalers | Verified-onchain |
...
| G-05 | The mix shifts between rounds while earlier holdings stay put                                          | A signal change checkpoints the elapsed stream first, then redirects later released revenue; it never reallocates past flow        | `Resonance.t.sol`; whitepaper abstract; ADR 0026                                                                 | Yes      | The protocol's defining property: the basket is a running sum of past distributions, not a portfolio anyone maintains                    | Verified-onchain |
| G-06 | Caption: "Nothing is sold, so the basket is every round so far."                                       | `Resonance._checkpointRevenue` (forward-looking only); `Fund.redeem` (the sole outward path)                                       | `Resonance.t.sol`, `Fund.t.sol`; ADR 0021, ADR 0026                                                              | Yes      | GBX held by Fund is separately burnable by anyone; that removes supply, not backing                                                      | Verified-onchain |
