...
9. Deploy LiquidityPosition with that exact pool, range, NFT ID, PositionManager, and Permit2. It is
   ownerless, and the NFT can never be recovered once delivered, so verify every parameter before transferring; then
...
11. Verify Resonance is already owned by the timelock, every immutable dependency and one-time binding matches, the
    factories cannot be rebound, GBX minting is locked to Fundraiser, and no external default admin remains. Fund and
    LiquidityPosition are ownerless and require no ownership step.

The frontend must remain read-only until a signed manifest proves chain ID, runtime bytecode, constructor arguments,
timelock roles and delay, minter lock, one-time bindings, PoolKey, ticks, NFT ID, and NFT custody. No such signed
manifest exists for this repository state.
