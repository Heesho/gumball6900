...
4. Deploy Voter with the timelock as owner.
5. Bind Voter once in SignalGBX and both factories.
6. Deploy VoterRouter and schedule Voter's one-time router binding through the timelock.
...
9. Deploy LiquidityPosition with that exact pool, range, NFT ID, PositionManager, VoterRouter, and timelock owner; then
