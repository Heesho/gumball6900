...
- Status: accepted for the development architecture; external governance remains unselected and deployment remains
  blocked; not independently audited or deployed; not approved for user funds
- Date: 2026-08-19
- Supersedes:
...
- Preserves: SignalGBX's non-transferable ERC20Votes checkpoints, mandatory signal backing, immediate exits, the three
  continuing Resonance administration methods, and the direct non-upgradeable core.

## Context

The repository implemented a bespoke OpenZeppelin Governor and Timelock solely to administer three Resonance methods.
...
Resonance remains the only owned core contract. Its continuing protocol administration methods remain:
...
Development and tests may keep the deployment fixture as Resonance's setup owner. A production deployment must create
and verify all one-time bindings and reviewed bootstrap Strategies, then transfer Resonance directly to the exact
external governance executor selected by a later ADR. The temporary setup owner must retain no authority afterward.
...
- A compromised external Resonance owner can misuse the three administration methods, transfer ownership, or renounce
  ownership. Ownerless contracts and immutable core parameters remain outside that authority.
