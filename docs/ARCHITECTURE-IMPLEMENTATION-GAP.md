...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| SignalGBX entry        | Idle-producing stake, allocation, removal, and combined workflows                                                        | Scalar and optional batched `addSignal`/`removeSignal`; no idle state, permit path, public move, or shared write Router             |
...
| Signal move surface    | Resonance exposed a dedicated `moveSignalFor` hook                                                                       | No dedicated Resonance hook or public SignalGBX move; wallets may compose direct remove/add calls atomically                        |
| Consumers              | ABI, SDK, subgraph, app, and references exposed superseded shapes                                                        | ADR 0051 adds direct write builders and a discovery-only position index; every state-sensitive write refreshes onchain              |
