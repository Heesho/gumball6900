...
Status reflects the local tree on 2026-08-16. “Fixed” means locally patched and regression-tested, not independently
...
Attacker prerequisites: A Strategy is killed with an active Bribe stream, every signaler exits, or rewards are notified after final exit.
...
Residual risk: Six days of the example stream plus every post-exit notification remain unreachable indefinitely.
...
Fixed locally: two High protocol mismatches, one High dependency advisory, and two Low campaign-infrastructure defects.
Dispositioned: one Informational analyzer report. Open and accepted: one Medium lifecycle risk. No undisclosed Critical or
High production-contract finding remains from this campaign.
