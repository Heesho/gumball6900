...
Evidence target: current development working tree on 2026-08-16. It is not release-pinned, deployed, independently
...
| S-02 | She deposits GBX into sGBX signals.                                           | `SignalGBX.signal` atomically transfers exact GBX, mints exact non-transferable sGBX, and adds the same paired-Bribe signal.        | Enforced; no idle sGBX or standalone staking path exists.                                                      |
...
| S-04 | An acquired-asset payment splits 90% to Fund and 10% to paired-Bribe rewards. | `BribeRouter` constants, cumulative `splitRemainder`, isolated liabilities, and Solidity/Python/TypeScript partition tests.         | Enforced and modeled; direct router donations remain surplus and additional explicit rewards remain possible.  |
| S-05 | A holder burns GBX and selects assets to receive.                             | `Fund.redeem` snapshots after Mine checkpointing and before burn, then atomically transfers the caller-selected unique non-GBX set. | Enforced; omitted assets are forfeited to remaining supply and a broken selected token reverts the basket.     |
| R-01 | Burn 1% of supply to receive 1% of each selected asset.                       | `floor(balance * gbxAmount / supplyBeforeBurn)` in `Fund.redeem`.                                                                   | Enforced up to integer floors; “1%” assumes the burn amount is exactly 1% of the checkpointed supply.          |
...
| R-03 | Nonempty handoffs split 80% to the displaced miner and 20% to Resonance.      | `PREVIOUS_MINER_BPS`, exhaustive remainder route, and independent models.                                                           | Enforced; an empty slot routes 100% to Resonance because no miner is displaced.                                |
| M-01 | 0% management fee.                                                            | No team, manager, or fee recipient exists; Mine's 80/20 and BribeRouter's 90/10 destinations are exact and build-pinned.            | Enforced as absence from this immutable surface; the Bribe leg rewards signalers and is not a management fee.  |
| M-02 | 0 team or presale tokens.                                                     | GBX constructor creates only the 20M genesis-liquidity allocation; Mine is the sole later minter after binding.                     | Enforced for mint destinations in code; completing the genesis-liquidity transfer is a deployment procedure.   |
...
| M-04 | 90% of each Strategy asset payment backs Fund.                                | `FUND_BPS = 9_000`, cumulative remainder accounting, and permissionless `payFundPayment`.                                           | Enforced as a fixed liability; settlement can remain pending while a token rejects Fund.                       |
| M-05 | 20M genesis GBX is locked in liquidity.                                       | `GBX.GENESIS_LIQUIDITY_ALLOCATION`; ownerless `LiquidityPosition` permanently retains the accepted NFT and fixed principal.         | Contract mechanics are enforced; delivery of the genesis allocation and NFT is procedural deployment evidence. |
...
`build.mjs` re-reads all core Solidity. It pins Mine's exact 80/20 classification, BribeRouter's exact cumulative 90/10
classification and remainder, the absence of privileged fee identifiers, protocol facts, WCAG contrast, stale claims,
word and page budgets, clipping, overlap, type size, embedded fonts, and PDF geometry before replacing published output.
