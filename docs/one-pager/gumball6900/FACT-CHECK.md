...
| G-05 | The mix shifts between rounds while earlier holdings stay put                                          | A signal change checkpoints the elapsed stream first, then redirects later released revenue; it never reallocates past flow        | `Resonance.t.sol`; whitepaper abstract; ADR 0025                                                                 | Yes      | The protocol's defining property: the basket is a running sum of past distributions, not a portfolio anyone maintains                    | Verified-onchain |
| G-06 | Caption: "Nothing is sold, so the basket is every round so far."                                       | `Resonance._checkpointRevenue` (forward-looking only); `Fund.redeem` (the sole outward path)                                       | `Resonance.t.sol`, `Fund.t.sol`; ADR 0021, ADR 0025                                                              | Yes      | GBX held by Fund is separately burnable by anyone; that removes supply, not backing                                                      | Verified-onchain |
