...
6. SignalGBX accepts stakes only after reciprocal Resonance binding, uses incremental absolute per-Strategy amounts,
   has no withdrawal lock, and lets users withdraw any unallocated balance immediately.
7. Resonance streams routed USDG globally through one active seven-day period and one aggregate successor using `1e36`
   exact quotient-plus-remainder accounting. Signal changes checkpoint prior elapsed flow and Strategy purchases
   atomically pull released USDG. ResonanceRouter forwards every nonzero complete balance; a live notification cannot
   change the active rate or finish. Old-weight carry that cannot be indexed before a signal mutation becomes explicit
   Fund remainder. Resonance creates uniform acquisition Strategies through bound factories. Every Strategy payment
   becomes a 100% fixed Fund liability; Bribes are funded independently.
...
11. Each Bribe has at most eight append-only reward tokens. Revenue and reward floor remainders remain explicit carry;
    old-supply Bribe carry and fully exiting user precision become fixed Fund classification before supply changes.
    Broken payout tokens do not block signal removal or unstaking.
