...
- Every reviewed initial Strategy was created before ownership handoff. For each Strategy, verify the payment token,
  ...
  protected final one.
- Verify one reviewed external fungible USDG/GBX LP ERC-20 is present as an ordinary bootstrap Strategy payment token.
...
| External LP asset | The reviewed USDG/GBX LP token remains an ordinary Strategy payment token; do not infer reserve value, price, or guaranteed market liquidity from core state.                                                                        |
...
A paid Mine handoff is operationally complete when Mine's `SafeERC20` request transfers the nominal protocol share to
