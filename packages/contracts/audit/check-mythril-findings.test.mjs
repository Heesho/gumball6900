...
const targetCount = REQUIRED_MYTHRIL_TARGETS.length;
...
test('runs and validates every clean Mythril target while preserving stdout, stderr, and exit codes', async (t) => {
  ...
  assert.equal(summary.targetCount, targetCount);
  assert.deepEqual(
...
test('rejects missing targets, stale bytecode hashes, and malformed archived JSON', async (t) => {
  ...
  await assert.rejects(
    ...
    new RegExp(`must contain exactly ${targetCount} expected targets`),
  );
...
test('zero-findings policy schema cannot silently add an accepted finding or remove a target', () => {
  ...
  assert.throws(() => validateMythrilPolicy(incomplete), new RegExp(`exactly ${targetCount} expected targets`));
...
test('checked-in Mythril policy remains the exact current-target zero-findings policy', async () => {
