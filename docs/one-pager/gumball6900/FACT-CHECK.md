...
| S-04 | The default payment split is 90% Fund / 10% paired Bribe; the Bribe share can be 0%-20%. | `Resonance.bribeBps`, its 2,000-bps maximum, BribeRouter's weighted `splitRemainder`, and cross-language transition tests.                   | Enforced and modeled prospectively; earlier liabilities and carry are not repriced.                            |
...
| M-04 | At least 80% of each Strategy asset payment backs Fund.                                  | `MAX_BRIBE_BPS = 2_000`, the exhaustive Fund complement, weighted carry, and permissionless `payFundPayment`.                                | Enforced for every new classification; the default Fund share is 90% and settlement can remain pending.        |
| M-06 | A 0% automatic rate leaves signaling and independent Bribes live.                        | `SignalGBX` paths do not depend on automatic liability; zero `notifyBribeReward` is a no-op; open Bribe funding remains.                     | Enforced by zero-rate signal/move/withdraw and independently funded reward tests.                              |
...
`build.mjs` re-reads all core Solidity. It pins Mine's exact 80/20 classification, Resonance's 10% default and 20%
maximum, BribeRouter's weighted cumulative classification and persistent remainder, the absence of privileged fee
