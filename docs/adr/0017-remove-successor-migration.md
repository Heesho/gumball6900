...
`Resonance` is now the only contract in the protocol with an owner. Its surface is `setBribeBps` (bounded at 50%),
`addStrategy`, `killStrategy`, and `addBribeReward` — the four actions ADR 0016 specified.
...
- Two failure modes previously mitigated by migration are now unmitigated by design, and are tracked as open items in
  ...
  notifications below the index resolution are absorbed by `Resonance` with no claim created.
